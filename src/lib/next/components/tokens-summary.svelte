<script lang="ts">
    ...
    import { SOL, USDC } from "$lib/next/constants";
    ...
    $: sorted = Array.from($balances.data?.values()).sort((a, b) =>
        a.amount < b.amount ? 1 : -1
    );

    const getBalanceSummary = (list: Balance[]) =>
        sorted.reduce((acc: any, balance) => {
            const solAdded = acc.find(({ id = "" } = {}) => id && id === SOL);

            const usdcAdded = acc.find(({ id = "" } = {}) => id && id === USDC);

            if (!solAdded && $balances.data.has(SOL)) {
                return [...acc, $balances.data.get(SOL)];
            }

            if (!usdcAdded && $balances.data.has(USDC)) {
                return [...acc, $balances.data.get(USDC)];
            }
...
<div class="min-h-full rounded-xl border p-4">
    <div class="col-span-5 mb-1 flex justify-between">
    ...
    {#each balanceSummary as _, idx}
        ...
        {#if $assets?.loading}
            <div class="flex">
                ...
                <div>
                    <div
                        class="h-4 w-[40%] animate-pulse rounded bg-secondary"
                    />
                    <div
                        class="h-2 w-[20%] animate-pulse rounded bg-secondary"
                    />
                </div>
            </div>
        {:else if balance}
