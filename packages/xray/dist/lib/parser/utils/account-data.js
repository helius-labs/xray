...
export const traverseAccountData = (accountData, accounts) => {
            ...
            if (data.tokenBalanceChanges &&
                ...
                for (let j = 0; j < data.tokenBalanceChanges.length; j++) {
                    ...
                    if (tokenBalanceData.rawTokenAmount.decimals === 0) {
                        amount = parseInt(tokenBalanceData.rawTokenAmount.tokenAmount);
                    }
                    else {
                        amount =
                            parseInt(tokenBalanceData.rawTokenAmount.tokenAmount) /
...
const indexChecker = (accounts, index, account, amount, mint) => {
    if (index !== -1) {
